...
GifForge creates a local backend account automatically with `POST /v1/auth/anonymous` and stores the returned access/refresh tokens in the shared Keychain. Sign in with Apple is optional and explicitly used for account recovery: the containing app can link the current local account through `POST /v1/auth/apple/link`, or restore an existing Apple-linked account through `POST /v1/auth/apple`. The Messages extension uses the shared Keychain token to call protected APIs; it does not own purchase UI or account recovery UI.

Apple In-App Purchase is the only payment provider. V1 uses consumable credit packs with product ids `dev.ericslutz.gifforge.credits.10`, `dev.ericslutz.gifforge.credits.25`, and `dev.ericslutz.gifforge.credits.55`. The client sends StoreKit's signed transaction JWS to `POST /v1/iap/transactions`; the backend verifies the JWS signature/certificate chain, bundle id, product id, transaction type, app account token, and revocation state before granting credits. StoreKit transactions are finished client-side only after the backend confirms the idempotent grant.

Azure SQL is the source of truth for local and optional Apple-linked user identity, refresh tokens, IAP products, IAP transactions, credit reservations, immutable ledger entries, generation ownership, and auth/purchase audit records. Azure Table Storage remains the operational store for generation job state plus short-lived App Attest challenge/session state.

Credit accounting uses reservation then capture. When a generation request is accepted, the backend estimates the selected provider/model route, SQL reserves the required credits, and records generation ownership before queueing the job. Current launch costs are 1 credit for standard fal.ai text/image generations, 2 credits for fal.ai video-to-video or Luma text/image fallback, and 5 credits for Luma video-to-video. The reservation reduces available balance, preventing a user from starting more concurrent jobs than they can afford. The worker captures the reservation only after a usable provider result is stored. Terminal provider/backend failure or job expiry releases the reservation, so no compensating credit is needed. App Store refund/revoke notifications insert reversal ledger entries; if already-spent credits make the balance negative, new reservations are blocked until the available balance is non-negative again.
